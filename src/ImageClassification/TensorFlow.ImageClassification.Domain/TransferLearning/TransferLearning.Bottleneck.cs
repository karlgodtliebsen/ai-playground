...
namespace ImageClassification.Domain.TransferLearning
{
    public partial class ExtendedTransferLearning
    {
                ...
                for (var j = 0; j < categories.Length; j++)
                {
                    ...
                    foreach (var (index, unused_base_name) in enumerate(category_list))
                    {
                        ...
                        if (how_many_bottlenecks % 300 == 0)
                            logger.Information($"{how_many_bottlenecks} bottleneck files created.");
                    }
                }
            }
        }
            ...
            Tensor jpeg_data_tensor, Tensor decoded_image_tensor, Tensor resized_input_tensor, Tensor bottleneck_tensor)
        {
            ...
            logger.Information("Creating bottleneck at " + bottleneck_path);
            var image_path = get_image_path(image_lists, label_name, options.DataDir, index, category);
            if (!File.Exists(image_path))
                logger.Information($"File does not exist {image_path}");
            ...
            string image_dir, int index, string category)
        {
            if (!image_lists.ContainsKey(label_name))
                logger.Information($"Label does not exist {label_name}");

            var label_lists = image_lists[label_name];
            if (!label_lists.ContainsKey(category))
                logger.Information($"Category does not exist {category}");
            var category_list = label_lists[category];
            if (category_list.Length == 0)
                logger.Information($"Label {label_name} has no images in the category {category}.");
