...
public interface IOpenAiChatCompletionService
{
    ...
    Task<OneOf<(ChatChoice response, int promptTokens, int responseTokens), ErrorResponse>> GetChatCompletion(IList<ChatCompletionMessage> messages,
        Guid sessionId,
        string deploymentName,
        CancellationToken cancellationToken,
        bool addStandardSystemPrompt = true);
}
