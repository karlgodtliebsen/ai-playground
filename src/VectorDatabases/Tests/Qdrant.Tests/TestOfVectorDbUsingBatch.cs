...
public class TestOfVectorDbUsingBatch
{
    ...
    static TestOfVectorDbUsingBatch()
    {

        VectorDbTestFixture.Launch = true;//crude primitive way to override docker launch. should be done in a better way
    }
...
https://colab.research.google.com/github/qdrant/examples/blob/master/qdrant_101_getting_started/getting_started.ipynb#scrollTo=5ws2UoCZo8bW
      ...
      //};
        ...
        await CreateCollectionInVectorDb(vector.Length);
        ...
        var body = new BatchUpsertBody()
        {
            Batch = batch
        };

        var s = JsonSerializer.Serialize(body, serializerOptions);
        output.WriteLine(s);

        var result = await client.Upload(collectionName, batch, CancellationToken.None);
        result.Switch(
